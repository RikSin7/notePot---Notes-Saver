...
function Login() {
  ...
  const handleLogin = () => {
    ...
    for (let i = 0; i < unallowedChars.length; i++) {
      if (password.includes(unallowedChars[i])) {
        newErrors.password =
          "Password can only contain these special characters: @, #, %, &, $, * ";
        isValid = false;
        break;
      }
    }
  ...
  const handleSignup = () => {
    ...
    if (isValid) {
      try {
        dispatch(signup(user)); // Try to sign up
        navigate("/"); // Redirect to home page after successful signup
      } catch (error) {
        toast.error(error.message); // Show error message if signup fails
      }
    }
  };
  ...
  const handleReset = () => {
    ...
    for (let i = 0; i < unallowedChars.length; i++) {
      if (newPassword.includes(unallowedChars[i])) {
        newErrors.password =
          "Password can only contain these special characters: @, #, %, &, $, * ";
        isValid = false;
        break;
        ...
        <>
          ...
          {isSignup && (
            <>
              ...
              {errors.email !== "" && (
                <span className="text-[#b55454] w-[70vw] sm:w-[500px] px-4">
                  {errors.email}
                </span>
              )}
            </>
